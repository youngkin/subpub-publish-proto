...
test:
	...
	@$(COVERTOOL) -cover .eunit/*coverdata -output coverage.xml -src src/

package:
	rm -rfv $(DIR_NAME)
	mkdir -p $(DIR_NAME)
	cp -v -R src priv sbin include ebin deps deploy misc $(DIR_NAME)/
	cp -v bshell version clean compile dev.config LICENSE README.md rebar rebar.config shell Makefile $(DIR_NAME)/
